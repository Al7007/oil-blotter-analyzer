...
from pipeline.metrics import analyze, compute_drop_consistency, draw_overlay
...
from pipeline.types import Diagnostics, DropCandidateInfo, PipelineResult
...
def _merge_candidates(candidates: list, selected) -> tuple[list, int]:
    ...
    """Добавляет каплю по клику в список, если её там ещё нет."""
...
def _diagnostics_for_candidate(
    ...
    *,
    ...
    cropped_rgb, stain, mask = crop_drop(
        ...
        original_rgb, stain_full, candidate.mask, padding_ratio=padding_ratio

    )

    geometry = find_drop_geometry(stain, mask) or candidate.geometry
    ...
    return analyze(blotter_zones, inverted, rgb=cropped_rgb)
    ...
    *,
    ...
    if click_point is not None:
        ...
        if selected is None:
            ...
            raise ValueError("Не удалось выделить каплю в этой точке. Кликните ближе к центру пятна.")
        ...
        raise ValueError("Не удалось найти каплю. Кликните по капле на исходнике.")
    ...
    drop_consistency = compute_drop_consistency(all_diagnostics)
    ...
    return PipelineResult(
        ...
        drop_consistency=drop_consistency,
